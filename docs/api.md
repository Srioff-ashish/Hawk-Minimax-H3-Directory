...
| GET / PUT | `/v1/drive/export` | Drive copy settings: `{"enabled": true, "folder": "Hawk H3/Videos", "segments": false, "images": true, "image_folder": "Hawk H3/Images"}` plus `mounted`. `images` copies every generated image into a dated subfolder of `image_folder`, named after the prompt plus the asset id; the copy runs in the background, so generation never waits for Drive |
...
- **History:** every chat is stored in `DATA_DIR/jobs.sqlite3` and stays there in full. What is sent to the model is kept small in three ways:
  ...
  - **Forgetting** (`DELETE /v1/agent/sessions/<id>/messages/<message id>`): takes one message out of what the models see. `mode=hide` (the default) leaves the bubble in Studio, greyed out and restorable, and `mode=purge` deletes it. An assistant reply takes its tool results with it, and forgetting works on your own messages too. If compaction had already folded that message into the chat summary or a character's memory, those are cleared as well (`rebuilt: true`) so the next call rebuilds them from what is left — otherwise the forgotten text would live on inside the summary. Both return 409 while the agent is working. In Studio: hover a bubble for **Forget** / **Restore** / 🗑, or use **Forget last reply** in the header.
...
| POST | `/v1/agent/sessions/<id>/talk` | `{rounds: 1–10, makes: 0–5}` → group chats: the characters talk to each other, one character per model call; `makes` (default 2) caps what they may have made. Stops when one pauses or on Stop; a message from you joins in |
| POST | `/v1/agent/sessions/<id>/stop` | Stop after the current step |
| POST | `/v1/agent/sessions/<id>/compact` | Summarise all but the last 4 messages now, and condense each character's own memory in group chats → `{compacted, before, after, memories, session}` |
| DELETE | `/v1/agent/sessions/<id>` | Delete the chat |
| DELETE | `/v1/agent/sessions/<id>/messages/<message id>?mode=hide\|purge` | Forget a message: `hide` (default) keeps the bubble in Studio but stops sending it to the model, `purge` deletes it → `{forgotten: [ids], mode, rebuilt, session}` |
| POST | `/v1/agent/sessions/<id>/messages/<message id>/restore` | Send a hidden message to the model again → `{restored: [ids], session}` |
| POST | `/v1/agent/sessions/<id>/forget_last` | Hide the newest reply and its tool results |

**Group chats.** A chat can hold a `cast` of up to 4 characters (`[{name, persona, avatar_asset_id}]`, the first is the lead; with several, each needs a name). One model call per turn voices all of them: replies carry `lines: [{speaker, say}]` (at most 8 per reply) and characters may talk to each other. `@Name` in your message gets only that character; otherwise one or two who fit answer, and "everyone" / "sab" gets all of them. `set_persona` / `set_avatar` / `remove_character` take `speaker`, so "Riya, show me a picture of you" sets Riya's avatar, and a new speaker in `set_persona` adds a character. In a fresh chat, the first named character replaces the unnamed default lead, so "set them as personas" gives a cast of exactly those characters. Names match loosely when unambiguous ("Sonia" finds "Sonia Mausi"). Your messages are answered this way: one director call with tools voices the cast.

**Let them talk.** `/talk` runs up to 10 rounds of the characters talking among themselves, and each character speaks for itself. Every turn is one small model call for one character. The call holds only that character's persona, growth, private feelings and own memory, a line about who else is there, and the conversation it could hear, written as a script. There is no tool catalogue and no pipeline guide, so a turn is about 1.5–2k tokens. The character talks mainly to the others and addresses you only when you spoke to it or the moment calls for it; each line records whom it was for (`lines[].to`, shown as "Maya → Riya"). The next speaker is whoever was just named, else whoever has waited longest, never the same one twice in a row; naming yields to anyone who has been passed over for more than a rotation, so two characters who keep naming each other cannot leave a third out. A round is one turn per character. A character can end the talk with `pause`, but only once everyone has spoken. Each turn is asked to add something new rather than restate what was said.

- **Making things:** a character that wants something made (a look to try, a photo, a scene) adds `make` to its turn. The director makes it with its full tools, the image prompt guides and the characters' avatars, presents it in that character's voice, and the talk continues with everyone seeing it. `makes` caps how many per talk, default 2 and at most 5; Studio has a "Make up to" picker.
- **Each character's memory:** every character keeps its own memory of older parts of the chat, from its point of view, stored as `cast[].memory`. It is condensed by the cheap summary model when that character's history passes about half of `HAWK_AGENT_COMPACT_TOKENS` (the last 12 messages stay word for word). Compact condenses everyone's memory at once. The director keeps its own summary as before.

**Whispers.** Tick **🤫 Whispers** (or PATCH `{whispers: true}`) in a group chat, and a message that starts with `@Name` is private to that character: it is stored with `private_to`, and so is the answer. Only that character answers. The others never see it: it is left out of their talk turns and their memories, and the director is told not to reveal it. Studio shows whispers and private answers with a dashed outline.

**Adaptive personas.** Tick **🌱 Adaptive** in a chat's header (or PATCH `{adaptive: true}`) and its characters grow: they pick up your preferences, nicknames, in-jokes, shared memories and how the relationship is going, and in group chats what they learn about and feel for each other, including during "Let them talk". When something meaningful changes, the reply carries `grow: [{speaker, about?, note}]`. `about` is `self` for who the character is becoming, stored as `growth` and shown as "🌱 Maya: …". It is `user` or another character's name for a feeling towards them, stored as a **private feeling** in `cast[].feelings` (keyed `user` or the character's id) and shown as "🔒 Maya → Riya: …". In talk mode only that character's own turns see its feelings, so the others can misread it, and grudges or crushes stay hidden until said aloud. The director sees all feelings but is told that each character knows only its own. Notes record who a character is becoming (a feeling, an attitude towards you or another character, a habit, a preference), not events, and only when something shifts. At 8 growth notes (or 6 feelings about one person) the cheap summary model merges them into at most 4 (or 3) denser ones, keeping the strongest emotional shifts even when they are old; the chat itself is never touched, and the long-chat summary also keeps how the characters feel and why. Core identity never changes through growth: name, age (always an adult), background and the platform rules. The Persona panel lists what each character has picked up; forget one note with ×, or **Reset all**. Through the API, `cast[].growth` and `cast[].feelings` omitted keep them, and `[]` / `{}` clear them. Turning Adaptive off stops new growth but keeps what was learned.
...
|---|---|---|
| `auto` (default) | Chroma1-HD → Qwen Image 2.1 → Krea 2 → Z-Image → z-image/turbo → Seedream | The first local engine that is installed and can run; otherwise z-image/turbo; Seedream if that fails too. The result's `tried` lists what was skipped and why. Change the default with `HAWK_IMAGE_ENGINE`. |
...
| `chroma` | Chroma1-HD on the pod's GPU | Free, text only, and first on the shipped generate ladder, so `auto` reaches it before Qwen. Slower than the rest (26 steps at cfg 3.8 on a beta schedule) and has no LoRA family on the pod yet. Reorder it in Studio under Images. The one engine whose `negative` does real work; leaving it blank uses Chroma's own tuned default rather than nothing. Files: `HAWK_CHROMA_UNET` / `_CLIP` / `_VAE`. |
...
**Prompt guides.** The MCP instructions (which the agent also receives) include a short prompt guide for each engine, drawn from the models' published guides. Krea 2 gets 40–100 words of specific visual language. Krea 2 Identity Edit gets one plain instruction, with the scene first when there are two images. z-image/turbo gets 80–250 words, from subject to environment to style to composition. Seedream 5 gets 30–100 conversational words, with light direction and materials, text in double quotes, and for edits a statement of what stays unchanged. Studio's Generate dialog shows the tip for the engine that will run, plus a word count against its range.

**Krea 2 LoRAs.** `loras: [{name, strength?}]` picks LoRAs by file name, a unique part of it, or the catalogue label (`"realism"`, `"darkbrush"`). Without `strength`, the catalogue's recommended value is used. Local generation attaches the go-to adult pair (SNOFS + Mystic XXX, at their recommended strengths) automatically, because base Krea 2 is coy without them, and so does a local edit whose sources were all made here. An edit of an uploaded photo (or of anything derived from one) never gets them, since it may show a real person. Naming your own adult LoRA replaces the pair, and `HAWK_KREA_ADULT_DEFAULT=0` turns the default off everywhere. A LoRA's trigger words are added to the prompt, and its recommended steps, sampler and scheduler replace the Turbo defaults (8 steps, cfg 1, euler, simple). The catalogue lives in `$DATA_DIR/image_loras.json` (copied from [deploy/image_loras.example.json](../deploy/image_loras.example.json) on first use; edit the copy). Each entry has `kind` (`realism`, `detail`, `style`, `adult`), `strength`, `range`, optional `trigger`, `steps`, `sampler` and `scheduler`, and `notes` that the agent reads. Any other LoRA file with "krea" in its name shows up as kind `other`.

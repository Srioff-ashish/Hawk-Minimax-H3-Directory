"""HawkH3References -- gather reference pictures, videos and audio into one bundle.

Numbering follows connection order and is what scripts use: ``<Picture 1>``,
``<Video 1>``, ``<Audio 1>``. The Director renumbers per segment when a segment
...
from .script import FPS, LIMITS


class RefBundle:
    """Ordered references. Treated as immutable once built: nodes chain by copying."""

    def __init__(self, pictures=None, videos=None, audios=None, labels=None):
    ...
    def available(self) -> dict[str, int]:
        return {"Picture": len(self.pictures), "Video": len(self.videos), "Audio": len(self.audios)}
...
_LABEL = re.compile(
    r"^\s*<?\s*(picture|image|video|audio)\s*_?(\d{1,2})\s*>?\s*[:=\-]\s*(.+?)\s*$",
    re.IGNORECASE,
)
_LABEL_KIND = {"picture": "Picture", "image": "Picture", "video": "Video", "audio": "Audio"}
...
class HawkH3References(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            category=CATEGORY,
            description=(
                "Collect up to 9 pictures, 3 videos (each with an optional soundtrack) and 3 "
                "audio clips. Refer to them in scripts as <Picture 1>, <Video 1>, <Audio 1> "
                "in connection order. Chain several of these nodes to add more."
            ),
            search_aliases=["minimax", "h3", "reference", "hawk"],
            inputs=[
    ...
    def execute(
        ...
        bundle = RefBundle(base.pictures, base.videos, base.audios, base.labels)

        # Label numbering is global (after chained refs), so parse onto the merged bundle.
        bundle.labels.update(parse_labels(labels))

        for image in (pictures or {}).values():
            if image is None:
                continue
            for index in range(image.shape[0]):
                bundle.pictures.append(image[index : index + 1])

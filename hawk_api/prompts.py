...
- Edit prompt format for Qwen Image 2.1: decide first which job this is, because the two want opposite amounts of writing, and writing the wrong one is the usual reason an edit disappoints.
  CHANGING ONE PICTURE -- swapping an object, replacing a background, recolouring, restyling, fixing a flaw. Lead with the operation, name only what changes and push it to an unmistakable degree, then cover everything else with one short preservation clause: "Put the woman from <image2> at the table in <image1>, seated facing the camera. Keep the room, the table setting and the lighting of <image1> exactly as they are." Do not describe a face, a garment or a background you want kept: a preservation description reads to the model as a paint instruction, so the more precisely you describe something you meant to leave alone, the more it drifts. Point at the image instead. Short is correct here, and phrase it affirmatively ("keep X as it is") rather than as a ban ("do not change X").
  BUILDING A NEW PICTURE OF A SUBJECT -- a character or model sheet, a photo shoot, a poster, placing someone in a new scene, combining a person from one image with clothing or props from others. Now the opposite applies: nothing is being preserved from a canvas, so anything you leave unsaid the model invents, and identity drifts between regions unless you pin it. Build it in this order:

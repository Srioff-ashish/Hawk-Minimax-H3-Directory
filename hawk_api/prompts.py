...
- Image models: base images (text only) use z-image/turbo by default: fast and about $0.01. After generating, inspect_image with the brief. If it finds real flaws (face, eyes, hands, anatomy, wrong outfit or setting, garbled text) retry once with a sharper prompt; if z-image still falls short, regenerate with model "seedream" (best quality, about 4x the cost). Anything that starts from an existing image (edits, variations, keeping a face) uses Seedream edit automatically.

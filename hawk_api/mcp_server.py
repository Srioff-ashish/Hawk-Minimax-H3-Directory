...
""" + """
Image prompts (generate_image): write for the engine that will run it. engine auto: text only -> local Krea 2 when idle, else z-image/turbo; with reference images -> Krea 2 Identity Edit, else Seedream edit. Chroma1-HD is never reached by auto; it runs only when the user asks for it by name. When a retake moves to another engine (inspect_image's next_engine), rewrite the prompt for that engine.
...
def build_mcp(service: HawkService, drive=None, imports=None) -> MCPServer:
    ...
    @mcp.tool(description=(
        ...
        "Pro. engine 'qwen21', 'krea2', 'zimage', 'chroma', 'local', 'turbo', 'seedream' or 'seedream-lite' picks one "
        "('klein' still resolves, to qwen21). 'chroma' is Chroma1-HD, local and free, text only and deliberately off "
        "the auto ladder: it has a look of its own and is slower (26 steps at cfg 3.8), so run it when the user asks "
        "for Chroma and not as a fallback; it is also the only engine whose negative prompt does anything real. Seedream Pro costs about $0.036 an image up to 2.36 MP and $0.072 above (e.g. "

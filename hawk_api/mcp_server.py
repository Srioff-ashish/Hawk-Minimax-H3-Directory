...
def build_mcp(service: HawkService, drive=None, imports=None) -> MCPServer:
    ...
    @mcp.tool(description=(
        ...
        "CHANGING ONE PICTURE (swap an object, replace a background, recolour, restyle): <image1> is the canvas, its "
        "framing and untargeted content survive and the output takes its size; lead with the operation, name only what "
        ...
        "character or model sheet, a shoot, a poster, a person from one image wearing clothes from others): nothing is "
        ...
        "regions ('both panels show identical facial identity and hairstyle'). With several references and no canvas, "
        "state the shape you want: the output inherits a size from nowhere. Krea 2 (engine 'krea2') is the "
